...
#Region "Fields"
    ...
    Private m_Estado As String = String.Empty
    Private m_Prioridad As Integer = 0
    Private m_Codigo As String = String.Empty
    Private m_NombreCodigo As String = String.Empty
    Private m_HorasPendientes As Integer = 0
    Private m_ProduccionXHora As Integer = 0

    Private m_Planeado As Integer = 0
    Private m_Completado As Integer = 0
    Private m_Pendiente As Integer = 0

    Private m_PorcentajeCompletado As Integer = 0
...
#Region "Properties"
    ...
    Protected Friend ReadOnly Property OpenRecurrenceForm() As Boolean
        ...
        End Get
    End Property
    <DXDescription("DevExpress.XtraScheduler.UI.AppointmentRibbonForm,ReadOnly"), DXCategory(CategoryName.Behavior)>
...
#End Region
    ...
    Public Overridable Sub LoadFormData(appointment As Appointment)
        'CARGAR INFORMACION DESDE LA BASE DE DATOS
        If appointment.CustomFields("Planeado") <> Nothing Then
            m_Planeado = appointment.CustomFields("Planeado")
            spinPlaneado.EditValue = m_Planeado
        End If
        If appointment.CustomFields("Completado") <> Nothing Then
            m_Completado = appointment.CustomFields("Completado")
            spinCompletado.EditValue = m_Completado
        End If
        If appointment.CustomFields("Pendiente") <> Nothing Then
            m_Pendiente = appointment.CustomFields("Pendiente")
            spinPendiente.EditValue = m_Pendiente
        End If
        If appointment.CustomFields("Codigo") <> Nothing Then
            m_Codigo = appointment.CustomFields("Codigo")
            txtCodigo.EditValue = m_Codigo
        End If

        If appointment.CustomFields("Estado") <> Nothing Then
            m_Estado = appointment.CustomFields("Estado")
            slueEstado.EditValue = m_Estado
        End If

        If appointment.CustomFields("Prioridad") <> Nothing Then
            m_Prioridad = appointment.CustomFields("Prioridad")
            txtPrioridad.EditValue = m_Prioridad
        End If

        If appointment.CustomFields("NombreCodigo") <> Nothing Then
            m_NombreCodigo = appointment.CustomFields("NombreCodigo")
            txtNombre.EditValue = m_NombreCodigo
        End If

        If appointment.CustomFields("ProduccionXHora") <> Nothing Then
            m_ProduccionXHora = appointment.CustomFields("ProduccionXHora")
            spinProduccionHora.EditValue = m_ProduccionXHora
        End If

        If appointment.CustomFields("HorasPendientes") <> Nothing Then
            m_HorasPendientes = appointment.CustomFields("HorasPendientes")
            spinHorasPendientes.EditValue = m_HorasPendientes
        End If

        If appointment.CustomFields("PorcentajeCompletado") <> Nothing Then
            m_PorcentajeCompletado = appointment.CustomFields("PorcentajeCompletado")
            tbProgress.EditValue = m_PorcentajeCompletado
        End If
    ...
    Public Overridable Function SaveFormData(appointment As Appointment) As Boolean
        'GUARDAR INFORMACION A LA BASE DE DATOS
        Try
            appointment.CustomFields("Estado") = slueEstado.EditValue
            appointment.CustomFields("Prioridad") = txtPrioridad.EditValue
            appointment.CustomFields("Codigo") = txtCodigo.EditValue
            appointment.CustomFields("NombreCodigo") = txtNombre.EditValue
            appointment.CustomFields("Planeado") = spinPlaneado.EditValue
            appointment.CustomFields("Completado") = spinCompletado.EditValue
            appointment.CustomFields("Pendiente") = spinPendiente.EditValue
            appointment.CustomFields("ProduccionXHora") = spinProduccionHora.EditValue
            appointment.CustomFields("HorasPendientes") = spinHorasPendientes.EditValue
            appointment.CustomFields("PorcentajeCompletado") = tbProgress.EditValue
            Return True
    ...
    Public Overridable Function IsAppointmentChanged(appointment As Appointment) As Boolean
        'ACTUALIZAR INFORMACION A LA BASE DE DATOS
        Try
            Return True
        Catch ex As Exception
            mMensajes.msg_Error(ex.Message.ToString)
        ...
        'If m_Planeado = appointment.CustomFields("Planeado") Or _
        '    m_Completado = appointment.CustomFields("Completado") Then
        '    Return False
        'Else
        '    Return True
        'End If
    ...
    Private Function CancelCore() As Boolean
        ...
        If DialogResult <> System.Windows.Forms.DialogResult.Abort AndAlso Controller IsNot Nothing AndAlso Controller.IsAppointmentChanged() AndAlso Not Me.m_supressCancelCore Then
            Dim dialogResult__1 As DialogResult = ShowMessageBox(SchedulerLocalizer.GetString(SchedulerStringId.Msg_SaveBeforeClose), Controller.GetMessageBoxCaption(SchedulerStringId.Msg_SaveBeforeClose), MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning)
    ...
    Protected Overridable Sub OnPrintButton()
        Me.dvInfo.ExecCommand(DevExpress.XtraPrinting.PrintingSystemCommand.PrintDirect)
    End Sub
    ...
    Private Sub OutlookAppointmentForm_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Try
            If spinPlaneado.EditValue < 1 Then
                Exit Sub
            End If

            tbProgress.EditValue = (Val(spinCompletado.EditValue) * 100) / Val(spinPlaneado.EditValue)
